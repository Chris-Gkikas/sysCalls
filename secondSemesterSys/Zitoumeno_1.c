...
int main(int argc, char const *argv[]){
	...
	if ((inFile = open(argv[1],O_RDONLY)) == -1){ printf("\nERROR: Can't open input file!\n\n"); return -1; }
	if ((outFile = open(argv[2],O_WRONLY|O_CREAT,S_IRWXU)) == -1){ printf("\nERROR: Can't open/create output file!\n\n"); return -1; }
	while(( readBytes = read(inFile,&buff,1)) >0 ){
		...
		if(buff=='\n'){												//end of current line
		 ...
		 if (writeBytes != readBytes){ printf("ERROR: Unequal number of read/write bytes in line #%d \n\n",lines); return -1; }	//in case of cpu goes crazy
	}
